 ...
 * 資料來源：mockRounds（steps / current / rerun / round2 / table）
 */
...
function DivisionCard({
  ...
  const color = DIVISION_COLORS[name.charAt(0)] || '#8B1A1A'
...
export function RoundsPage() {
  const { steps, current, rerun, round2, table } = mockRounds
        ...
        <div className="px-6 pt-6 pb-6">
          <div className="grid grid-cols-5 gap-3">
            {current.divisions.map((d) => (
              <DivisionCard key={d.name} name={d.name} text={d.text} tie={d.tie} tieNote={d.tieNote} />
            ))}
          </div>
        </div>
      </Card>
      ...
      <div className="grid grid-cols-2 gap-6 mt-6">
        <Card className="pb-6">
          <CardHeader
            ...
            action={<StatusPill tone="wait">{rerun.status}</StatusPill>}
          />
          <InnerPanel>
            {rerun.rows.map((row) => (
        ...
        <Card className="pb-6">
          <CardHeader
            ...
            action={<StatusPill tone="idle">{round2.status}</StatusPill>}
          />
          <InnerPanel>
            {round2.rows.map((row) => (
              <KvRow key={row.k} k={row.k} v={row.v} />
